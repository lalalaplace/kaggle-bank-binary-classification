...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X_meta_oof, y), 1):
    ...
    best_iter = booster.best_iteration + 1 if booster.best_iteration is not None else booster.num_boosted_rounds()

    try:
        oof_meta[va_idx] = booster.predict(dva, iteration_range=(0, best_iter))
        test_meta += booster.predict(dte, iteration_range=(0, best_iter)) / FOLDS
    except TypeError:
        # 旧版 xgboost 没有 iteration_range
        ntree_limit = getattr(booster, "best_ntree_limit", 0)
        if ntree_limit == 0:
            ntree_limit = best_iter
        oof_meta[va_idx] = booster.predict(dva, ntree_limit=ntree_limit)
        test_meta += booster.predict(dte, ntree_limit=ntree_limit) / FOLDS
...
sub["y"] = test_meta
sub.to_csv("outputs/submissions/xgb_stacking.csv", index=False)
print("Saved:", "xgb_stacking.csv")
